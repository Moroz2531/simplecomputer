...
int command_encode(int sign, int command, int operand, int *value) {
...
static int memory_save(int *memory, char *filename) {
...
static int memory_load(int *memory, Var *var) {
  while (var->next != NULL)
    var = var->next;
...
static char *add_extension_for_file(char *src) {
...
static void print_error(int err, int pos_string) {
  switch (err) {
  case ERR_OPERAND:
...
int translator_simple_assembler(char *filename) {
